...
contract selectiveTender {
    ...
    function addBidder(string memory _name, address _addr) public onlyOfficial {
    ...
    function bid(uint _tdrID, string memory _name) public payable {
        ...
        require(
            bytes(bidders[msg.sender].bidderName).length != 0,
            "Not in bidders list"
        );
    ...
    function selectWinner(uint _tdrID, uint _bidID) public payable {
        require(tdrs[_tdrID].endTime < block.timestamp, "Bid has not ended.");
    ...
    function getWinningBid(
        ...
        require(tdrs[_tdrID].endTime < block.timestamp, "Bid has not ended.");
        return (
            tdrs[_tdrID].bidders[tdrs[_tdrID].winningBid],
            bidders[tdrs[_tdrID].bidders[tdrs[_tdrID].winningBid]].bidAmt[
                _tdrID
            ]
        );
    }
    ...
    function bidInfo(
        uint _tdrID,
        uint _bidID
    ) public view returns (address bidderAddress, uint bidAmtt) {
        return (
